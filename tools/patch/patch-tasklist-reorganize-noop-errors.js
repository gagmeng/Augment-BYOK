...
function patchTasklistReorganizeNoopErrors(filePath) {
  ...
  next = replaceOnceRegex(
    next,
    /((?:let|const)\s+([A-Za-z_$][\w$]*)=[A-Za-z_$][\w$]*\.markdown;if\(!\2\)return\s+([A-Za-z_$][\w$]*)\("No markdown provided\."\);[\s\S]*?)(if\(!([A-Za-z_$][\w$]*)\)return\s+\3\("Failed to retrieve updated task tree(?: after reorganization)?\."\);\s*(?:let|const)\s+([A-Za-z_$][\w$]*)=([A-Za-z_$][\w$]*)\.formatBulkUpdateResponse\(([A-Za-z_$][\w$]*)\(([A-Za-z_$][\w$]*),([A-Za-z_$][\w$]*)\)\);\s*return\s*((?:[A-Za-z_$][\w$]*&&[A-Za-z_$][\w$]*\([^)]*\),)?)\{\.\.\.([A-Za-z_$][\w$]*)\(\6\),plan:\5\})/g,
    (m) => {
