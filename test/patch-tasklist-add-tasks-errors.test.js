...
test("patchTasklistAddTasksErrors: patches handleBatchCreation with non-Qk diff helper", () => {
  withTempDir("augment-byok-task-errors-", (dir) => {
    ...
    const src = [
      ...
      `    if(!i)return et("No task list found. [TL005]");`,
      "    let o=await this._taskManager.getHydratedTask(i);",
      "    if(!o)return et(`Task with UUID ${i} not found.`);",
      "    let s=[];",
      ...
      "    if(!a)return et(\"Failed to retrieve updated task tree.\");",
      "    let c=p1.formatBulkUpdateResponse(kF(o,a));",
      "    return{...nr(c),plan:a}",
      "  }",
      "}"
    ].join("\n");
    ...
    assert.ok(out1.includes('return{...et("Failed to add task(s)."+__byok_msg),plan:a}'));
    assert.ok(out1.includes("let c=p1.formatBulkUpdateResponse(kF(o,a));let __byok_failed=s.filter"));
