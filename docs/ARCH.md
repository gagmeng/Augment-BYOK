...
目标：**最小破坏面 + 可审计 + 可回滚**（只接管 7 个 LLM 数据面端点）。
...
- Acceptance：BYOK 关闭立即回到官方链路；BYOK 开启时 7 个 LLM 数据面端点按路由工作（见 `docs/ENDPOINTS.md`）。
...
- 端点覆盖范围：仅 7 个 LLM 数据面端点有 BYOK 语义实现（见 `docs/ENDPOINTS.md`）
