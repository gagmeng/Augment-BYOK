...
- 确认 Endpoint Rules 对目标端点为 `byok`（面板默认已覆盖 7 个）
...
- 严格检查：`npm run check:official-delegation -- --require-all`（要求 6 个“官方拼接端点”（除 `/get-models`）全部出现且 miss/fail=0）
